...
public class StructuresTest {
    ...
    public void setUp() throws Exception {
        // get xml-based application context
    }

    @Test
    public void testStructures() throws Exception {
        final String contextConfigPath = getClass().getResource("/project/project003.xml").getFile();
        applicationContext.loadBeanDefinitions(new XmlResource(contextConfigPath));
        // load and check
        final ComplexObject complexObject = applicationContext.getBean("complexObject", ComplexObject.class);
        // check collection
        assertNotNull("Can't load object from context", complexObject);
        assertNotNull("Can't load list collection from context", complexObject.getStringCollection());
        assertFalse("Can't load items from collection", complexObject.getStringCollection().isEmpty());
