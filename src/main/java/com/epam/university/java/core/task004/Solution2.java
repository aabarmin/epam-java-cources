...
//             };
    ...
    private String[] unionHelpFunction(String[] firstNoDublicates, String[] secondNoDublicates) {

        //нахожу количество одинаковых элементов массивов без дубликатов
        int count = 0;
        for (int i = 0; i < firstNoDublicates.length; i++) {
            for (int j = 0; j < secondNoDublicates.length; j++) {
                if (firstNoDublicates[i].equals(secondNoDublicates[j])) {
                    count++;
                }
            }
        }

        String[] result = new String [firstNoDublicates.length + secondNoDublicates.length - count];
        ...
        for (int i = 0; i < firstNoDublicates.length; i++) {
            ...
            for (int j = 0; j < result.length; j++) {
                if (firstNoDublicates[i].equals(result[j])) {
                    indicator = 1;
                }
            }
            if (indicator == 0) {
                result[k] = firstNoDublicates[i];
                k++;
            }
        }

        //добавляем элементы второго массива, которых нет в первом массиве
        for (int i = 0; i < secondNoDublicates.length; i++) {
            indicator = 0;
            for (int j = 0; j < result.length; j++) {
                if (secondNoDublicates[i].equals(result[j])) {
                    indicator = 1;
                }
            }
            if (indicator == 0) {
                result[k] = secondNoDublicates[i];
                k++;
            }
        }
