...
public class Task006Impl implements Task006 {
  ...
  public double resistance(Collection<Measurement> measurements) {
    ...
    double resistance = (length * sumVA - sumA * sumV) / (length * sumASquare - Math.pow(sumA, 2));

    return (double) (int) (resistance * 1000) / (double) 1000;
  }

  private double adder(Collection<Measurement> measurements, ToDoubleFunction<Measurement> lambda) {
