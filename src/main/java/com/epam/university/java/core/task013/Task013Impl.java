...
public class Task013Impl implements Task013 {
    ...
    public Figure invokeActions(Figure figure, Collection<FigureAction> actions) {
        Validator.validateNotNull(figure, actions, Validator.MESSAGE_FOR_FIRST_PARAMETER_IF_NULL,
    ...
    public boolean isConvexPolygon(Figure figure) {
        ...
        List<Vertex> inputVertexList = new ArrayList<>(figure.getVertexes());
        Point2D[] points = new Point2D[inputVertexList.size()];
        for (int i = 0; i < inputVertexList.size(); i++) {
            points[i] = new Point2D(inputVertexList.get(i).getX(), inputVertexList.get(i).getY());
        }
        GrahamScan graham = new GrahamScan(points);
        if (graham.getHull().size() == inputVertexList.size() && graham.isConvex()) {
            // prints the extreme points on the convex hull in counterclockwise order
            graham.getHull().stream().map(point2D -> new VertexImpl((int) point2D.x(), (int) point2D.y()))
