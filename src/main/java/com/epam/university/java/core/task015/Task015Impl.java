...
public class Task015Impl implements Task015 {
    ...
    public double getArea(Square first, Square second) {
        ...
        List<Point> firstSquarePoints = Geo.getAllSquarePoints(first);
        List<Point> secondSquarePoints = Geo.getAllSquarePoints(second);
        // Could be no intersection points, if one square inside another
        List<Point> intersectionPoints = Geo.getSquaresIntersectionPoints(firstSquarePoints,
                secondSquarePoints);

        Set<Point> intersectionPolygonPoints = new HashSet<>();
        intersectionPolygonPoints.addAll(intersectionPoints);
        intersectionPolygonPoints.addAll(Geo.getSquareInternalPoints(firstSquarePoints,
                secondSquarePoints));
        intersectionPolygonPoints.addAll(Geo.getSquareInternalPoints(secondSquarePoints,
                firstSquarePoints));
        ...
        List<Point> clockwisePoints = Geo.getClockwiseOrderedPolygonPoints(polygonPoints);
