...
public class Task005Impl implements Task005 {
    ...
    public PiHolder findPi(int digits) {
        ...
        Optional<PiHolderImpl> min = Arrays.stream(range)
                .mapToObj(n -> new PiHolderImpl(n,
                        (int) (n/PI)+1,
                        Math.abs(PI-(double)n/((int)(n/PI)+1))))
                .flatMap(n -> Stream.of(new PiHolderImpl[]{
                        n,
                        new PiHolderImpl(n.getFirst(),
                                n.getSecond()-1,
                                Math.abs(PI-(double)n.getFirst()/((int)(n.getSecond()-1))))
                }))
